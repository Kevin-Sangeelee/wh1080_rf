...
uint16_t cmd_freq	= CMD_FREQ|0x620; // 433.92 MHz
	...
	uint16_t cmd_config	= CMD_CONFIG|BAND_433|LOAD_CAP_12C0|BW_134;
...
//	return -1;
	...
	*(gpio.addr + (0x28 >> 2)) = 1 << 22;
	...
	// Current and previous data-pin values, used to detect transitions.
	uint8_t datapin = 0, olddatapin = 0;

	// Timestamps used to calculate the pulse widths.
	unsigned int datatime, olddatatime;
	
	// Counters for short packets (to report presence of noise), bit-count,
	// and timeout...
	...
	//  Buffer of timestamps @ data-pin transitions
	unsigned int datatime_buf[500];
	
	unsigned char bytes[10];

	unsigned int now;
	
	olddatatime = getTimer();

	// Switch to realtime scheduler
	scheduler_realtime();

/*
	// Test code for sign of timer increment ripple bug
	unsigned long xcount = 5000000000;
...
*/
	...
	do {
		...
		datapin = ((*(gpio.addr + 13)) >> 21) & 1;

		datatime = getTimer();
		// Check if the pin transitioned
		if(datapin != olddatapin) {
			// If falling edge (1 -> 0), then store bit pulse duration
			if(datapin == 0) {
				datatime_buf[count] = datatime - olddatatime;
				if(++count == 500)
				count = 499;
			}
			olddatapin = datapin;
		...
		int packet_offset = 0;
		now = getTimer();
		if(!timeout && (now - olddatatime) > 5000) { // && count > 0
			...
			if(count > 60) { // then maybe something at least interesting
				...
				for(idx=0; idx < count; idx++) {
					bit = datatime_buf[idx] < g_low_threshold ? 1 : 0;
					sig_in = (sig_in << 1) | bit;
					...
					*(gpio.addr + (0x1c >> 2)) = 1 << 22;
					...
					for(idx=0; idx < 10; idx++) {
						...
						for(b=0; b < 8; b++) {
							...
							uint8_t bit = datatime_buf[packet_offset + (idx * 8 + b)] < g_low_threshold ? 1 : 0;
							byte = (byte << 1) + bit;
						}
						bytes[idx] = byte;
						printf("%02x ", byte);
			...
			// If we get enough bits, then dump stats to indicate pulse lengths coming from the device. This information
			// might help debug and investigate reception problems...
			if(count > 40) {
				...
				if(count == 88 && crc_passed) {
					...
					unsigned int wait_start = getTimer(), elapsed;
					...
					do {
						elapsed = (getTimer() - wait_start) / 1000000;
